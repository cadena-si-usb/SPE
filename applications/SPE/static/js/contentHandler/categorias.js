...
$('#categoriasIndex').ready(function(){
    ...
    function getInfo(){
        ...
        ajaxHandler.count('categorias',options).success(function(res){
            max = res;
    ...
    function getCategorias(){
        ajaxHandler.find('categorias',options).success(function(res){
            categoria = JSON.parse(res);
            if (categoria.length > 0){
                $("#elBody").loadTemplate('#template', categoria);
            }
            else {
    ...
    // Funcion que trae mas categoria para paginacion

    function getMoreCategorias(evt){
        filters.page = categoria.length.toString();
    ...
    function disableButtonOnMax(){
        if ((max) && (categoria.length.toString() != max)) {
