...
$('#areas_proyectoIndex').ready(function(){
    ...
    function getInfo(){
        ...
        ajaxHandler.count('areas_proyecto',options).success(function(res){
            max = res;
    ...
    function getAreas_Proyecto(){
        ajaxHandler.find('areas_proyecto',options).success(function(res){
            area_proyecto = JSON.parse(res);
            if (area_proyecto.length > 0){
                $("#elBody").loadTemplate('#template', area_proyecto);
            }
            else {
    ...
    // Funcion que trae mas area_proyecto para paginacion

    function getMoreAreas_Proyecto(evt){
        filters.page = area_proyecto.length.toString();
    ...
    function disableButtonOnMax(){
        if ((max) && (area_proyecto.length.toString() != max)) {
    ...
    $('#next').click(getMoreAreas_Proyecto);
