...
$('#carrerasIndex').ready(function(){
    ...
    function getInfo(){
        ...
        ajaxHandler.count('carreras',options).success(function(res){
            max = res;
    ...
    function getCarreras(){
        ajaxHandler.find('carreras',options).success(function(res){
            carrera = JSON.parse(res);
            if (carrera.length > 0){
                $("#elBody").loadTemplate('#template', carrera);
            }
            else {
    ...
    // Funcion que trae mas carrera para paginacion

    function getMoreCarreras(evt){
        filters.page = carrera.length.toString();
    ...
    function disableButtonOnMax(){
        if ((max) && (carrera.length.toString() != max)) {
