...
$('#divisionesIndex').ready(function(){
    ...
    function getInfo(){
        ...
        ajaxHandler.count('divisiones',options).success(function(res){
            max = res;
    ...
    function getDivisions(){
        ajaxHandler.find('divisiones',options).success(function(res){
            division = JSON.parse(res);
            if (division.length > 0){
                $("#elBody").loadTemplate('#template', division);
            }
            else {
    ...
    // Funcion que trae mas division para paginacion

    function getMoreDivisions(evt){
        filters.page = division.length.toString();
    ...
    function disableButtonOnMax(){
        if ((max) && (division.length.toString() != max)) {
