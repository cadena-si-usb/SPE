...
$('#universidadesIndex').ready(function(){
    ...
    function getInfo(){
        ...
        ajaxHandler.count('universidades',options).success(function(res){
            max = res;
    ...
    function getUniversidads(){
        ajaxHandler.find('universidades',options).success(function(res){
            universidad = JSON.parse(res);
            if (universidad.length > 0){
                $("#elBody").loadTemplate('#template', universidad);
            }
            else {
    ...
    // Funcion que trae mas universidad para paginacion

    function getMoreUniversidads(evt){
        filters.page = universidad.length.toString();
    ...
    function disableButtonOnMax(){
        if ((max) && (universidad.length.toString() != max)) {
    ...
    $('#next').click(getMoreUniversidads);
