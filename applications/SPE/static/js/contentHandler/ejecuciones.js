...
$('#ejecucionesIndex').ready(function(){
    ...
    function getInfo(){
        ...
        ajaxHandler.count('ejecuciones',options).success(function(res){
            max = res;
    ...
    function getEjecucions(){
        ajaxHandler.find('ejecuciones',options).success(function(res){
            ejecucion = JSON.parse(res);
            if (ejecucion.length > 0){
                $("#elBody").loadTemplate('#template', ejecucion);
            }
            else {
    ...
    // Funcion que trae mas ejecucion para paginacion

    function getMoreEjecucions(evt){
        filters.page = ejecucion.length.toString();
    ...
    function disableButtonOnMax(){
        if ((max) && (ejecucion.length.toString() != max)) {
