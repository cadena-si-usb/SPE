...
$('#paisesIndex').ready(function(){
    ...
    function getInfo(){
        ...
        ajaxHandler.count('paises',options).success(function(res){
            max = res;
    ...
    function getPaiss(){
        ajaxHandler.find('paises',options).success(function(res){
            pais = JSON.parse(res);
            if (pais.length > 0){
                $("#elBody").loadTemplate('#template', pais);
            }
            else {
    ...
    // Funcion que trae mas pais para paginacion

    function getMorePaiss(evt){
        filters.page = pais.length.toString();
    ...
    function disableButtonOnMax(){
        if ((max) && (pais.length.toString() != max)) {
