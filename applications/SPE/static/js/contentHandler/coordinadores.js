...
$('#coordinadoresIndex').ready(function(){
    ...
    function getInfo(){
        ...
        ajaxHandler.count('coordinadores',options).success(function(res){
            max = res;
    ...
    function getCoordinadores(){
        ajaxHandler.find('coordinadores',options).success(function(res){
            coordinador = JSON.parse(res);
            if (coordinador.length > 0){
                $("#elBody").loadTemplate('#template', coordinador);
            }
            else {
    ...
    // Funcion que trae mas coordinador para paginacion

    function getMoreCoordinadores(evt){
        filters.page = coordinador.length.toString();
    ...
    function disableButtonOnMax(){
        if ((max) && (coordinador.length.toString() != max)) {
    ...
    $('#next').click(getMoreCoordinadores);
