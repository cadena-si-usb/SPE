...
$('#materias_periodoIndex').ready(function(){
    ...
    function getInfo(){
        ...
        ajaxHandler.count('materias_periodo',options).success(function(res){
            max = res;
    ...
    function getMateria_Periodos(){
        ajaxHandler.find('materias_periodo',options).success(function(res){
            materia_periodo = JSON.parse(res);
            if (materia_periodo.length > 0){
                $("#elBody").loadTemplate('#template', materia_periodo);
            }
            else {
    ...
    // Funcion que trae mas materia_periodo para paginacion

    function getMoreMateria_Periodos(evt){
        filters.page = materia_periodo.length.toString();
    ...
    function disableButtonOnMax(){
        if ((max) && (materia_periodo.length.toString() != max)) {
    ...
    $('#next').click(getMoreMateria_Periodos);
