...
$('#coordinacionesIndex').ready(function(){
    ...
    function getInfo(){
        ...
        ajaxHandler.count('coordinaciones',options).success(function(res){
            max = res;
    ...
    function getCoordinaciones(){
        ajaxHandler.find('coordinaciones',options).success(function(res){
            coordinacion = JSON.parse(res);
            if (coordinacion.length > 0){
                $("#elBody").loadTemplate('#template', coordinacion);
            }
            else {
    ...
    // Funcion que trae mas coordinacion para paginacion

    function getMoreCoordinaciones(evt){
        filters.page = coordinacion.length.toString();
    ...
    function disableButtonOnMax(){
        if ((max) && (coordinacion.length.toString() != max)) {
    ...
    $('#next').click(getMoreCoordinaciones);
