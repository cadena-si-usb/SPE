...
$('#profesoresIndex').ready(function(){
    ...
    function getInfo(){
        ...
        ajaxHandler.count('profesores',options).success(function(res){
            max = res;
    ...
    function getProfesors(){
        ajaxHandler.find('profesores',options).success(function(res){
            profesor = JSON.parse(res);
            if (profesor.length > 0){
                $("#elBody").loadTemplate('#template', profesor);
            }
            else {
    ...
    // Funcion que trae mas profesor para paginacion

    function getMoreProfesors(evt){
        filters.page = profesor.length.toString();
    ...
    function disableButtonOnMax(){
        if ((max) && (profesor.length.toString() != max)) {
