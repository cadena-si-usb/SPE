...
$('#etapasIndex').ready(function(){
    ...
    function getInfo(){
        ...
        ajaxHandler.count('etapas',options).success(function(res){
            max = res;
    ...
    function getEtapas(){
        ajaxHandler.find('etapas',options).success(function(res){
            etapa = JSON.parse(res);
            if (etapa.length > 0){
                $("#elBody").loadTemplate('#template', etapa);
            }
            else {
    ...
    // Funcion que trae mas etapa para paginacion

    function getMoreEtapas(evt){
        filters.page = etapa.length.toString();
    ...
    function disableButtonOnMax(){
        if ((max) && (etapa.length.toString() != max)) {
