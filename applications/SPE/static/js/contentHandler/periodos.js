...
$('#periodosIndex').ready(function(){
    ...
    function getInfo(){
        ...
        ajaxHandler.count('periodos',options).success(function(res){
            max = res;
    ...
    function getPeriodos(){
        ajaxHandler.find('periodos',options).success(function(res){
            periodo = JSON.parse(res);
            if (periodo.length > 0){
                $("#elBody").loadTemplate('#template', periodo);
            }
            else {
    ...
    // Funcion que trae mas periodo para paginacion

    function getMorePeriodos(evt){
        filters.page = periodo.length.toString();
    ...
    function disableButtonOnMax(){
        if ((max) && (periodo.length.toString() != max)) {
