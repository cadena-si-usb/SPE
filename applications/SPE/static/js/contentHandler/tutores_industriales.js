...
$('#tutores_industrialesIndex').ready(function(){
    var ajaxHandler = AjaxHandler();

    var tutor_industrial = [],
        max, area, options = {},
    ...
    function getInfo(){
        ...
        ajaxHandler.count('tutores_industriales',options).success(function(res){
            max = res;
            $('#cantidad').html(max.toString());
            getTutor_Industrials();
        })
    }

    function getTutor_Industrials(){
        ajaxHandler.find('tutores_industriales',options).success(function(res){
            tutor_industrial = JSON.parse(res);
            if (tutor_industrial.length > 0){
                $("#elBody").loadTemplate('#template', tutor_industrial);
            }
            else {
    ...
    // Funcion que trae mas tutor_industrial para paginacion

    function getMoreTutor_Industrials(evt){
        filters.page = tutor_industrial.length.toString();
    ...
    function disableButtonOnMax(){
        if ((max) && (tutor_industrial.length.toString() != max)) {
    ...
    $('#next').click(getMoreTutor_Industrials);
