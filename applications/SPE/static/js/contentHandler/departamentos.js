...
$('#departamentosIndex').ready(function(){
    ...
    function getInfo(){
        ...
        ajaxHandler.count('departamentos',options).success(function(res){
            max = res;
    ...
    function getDepartamentoes(){
        ajaxHandler.find('departamentos',options).success(function(res){
            departamento = JSON.parse(res);
            if (departamento.length > 0){
                $("#elBody").loadTemplate('#template', departamento);
            }
            else {
    ...
    // Funcion que trae mas departamento para paginacion

    function getMoreDepartamentoes(evt){
        filters.page = departamento.length.toString();
    ...
    function disableButtonOnMax(){
        if ((max) && (departamento.length.toString() != max)) {
    ...
    $('#next').click(getMoreDepartamentoes);
