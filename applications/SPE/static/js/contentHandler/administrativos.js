...
$('#administrativosIndex').ready(function(){
    ...
    function getInfo(){
        ...
        ajaxHandler.count('administrativos',options).success(function(res){
            max = res;
    ...
    function getAdministrativos(){
        ajaxHandler.find('administrativos',options).success(function(res){
            administrativos = JSON.parse(res);
            console.log(administrativos)
            if (administrativos.length > 0){
                $("#elBody").loadTemplate('#template', administrativos);
            }
            else {
    ...
    // Funcion que trae mas administrativos para paginacion

    function getMoreAdministrativos(evt){
        filters.page = administrativos.length.toString();
    ...
    function disableButtonOnMax(){
        if ((max) && (administrativos.length.toString() != max)) {
    ...
    $('#next').click(getMoreAdministrativos);
