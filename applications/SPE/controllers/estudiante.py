...
def registrar_estudiante():
    ...
    db.UsuarioUSB.usbid.default  = request.vars.usbid

    # ('Docente','Administrativo','Pregrado','Postgrado','Organizacion','Egresado'))]
    if usuario.get('tipo') == 'Pregrado':
        db.Estudiante.carnet.default = request.vars.usbid
        db.Estudiante.carnet.writable = False

    db.UsuarioUSB.nombre.default = usuario.get('first_name')
    db.UsuarioUSB.apellido.default = usuario.get('last_name')
    db.UsuarioUSB.tipo_documento.default = 'CI'
    db.UsuarioUSB.numero_documento.default = usuario.get('cedula')

    db.UsuarioUSB.usbid.writable  = False
    db.Estudiante.carrera.default  = usuario['carrera']
    ...
    if form.process().accepted:
        ...
        result = db.UsuarioUSB.insert(
            ...
            direcUsuario=request.vars.direcUsuario,
            telefono=request.vars.telefono,
        )

        usuarioUSBSet = db(db.UsuarioUSB, db.UsuarioUSB.usbid == request.vars.usbid).select()
        usuarioUSB = usuarioUSBSet[0]

        # Ingresamos a la base de datos de Estudiante
        result = db.Estudiante.insert(
            usuario=usuarioUSB.id,
    ...
    return response.render('Estudiante/rregistro.html',
                           message=T("Por favor actualiza tus datos para continuar"), form=form)
