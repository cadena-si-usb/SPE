...
def sqlform_grid():
    sqlform_grid = single_table_spe_grid(db.Inscripcion)
    return sqlform_grid
...
def get():
    ...
    rows = db((db.Inscripcion.pasantia == db.Pasantia.id) & (db.Pasantia.estudiante == db.Estudiante.id) & (db.Pasantia.id == db.Plan_Trabajo.pasantia)).select()
