...
def Area_Laboral_Table(db, T):
    db.define_table('Area_Laboral',
                    Field('first_name', 'string', required=True, ondelete='CASCADE',
                          notnull=True, label='Area Laboral', unique=True),
    ...
    db.Area_Laboral.first_name.requires += [
        IS_NOT_IN_DB(db, 'Area_Laboral.first_name', error_message=T('Area Laboral ya registrado'))]
